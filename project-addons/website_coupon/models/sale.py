    ...
    def _cart_update(self, cr, uid, ids, product_id=None, line_id=None, add_qty=0, set_qty=0, context=None, **kwargs):
        ...
        for so in self.browse(cr, uid, ids, context=context):
            if line_id != False:
                ...
                if line_ids:
                    line = sol.browse(cr, SUPERUSER_ID, line_ids[0], context=context)
                    if line.coupon_discount_line:
                        allow_update = False
        if allow_update:
            return super(SaleOrder, self)._cart_update(cr, uid, ids, product_id, line_id, add_qty, set_qty, context, **kwargs)
        else:
